YUI.add('moodle-mod_aiquiz-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for aiquiz sections and slots.
 *
 * @module moodle-mod-aiquiz-dragdrop
 */
 ...
 *
 * @class M.mod_aiquiz.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'moodle-mod_aiquiz-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_aiquiz-dragdrop');
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_aiquiz.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {
                        // Ignore.
                    }
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                M.mod_aiquiz.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_aiquiz-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_aiquiz.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_aiquiz.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_aiquiz.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_aiquiz.quizbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_aiquiz.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_aiquiz-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_aiquiz = M.mod_aiquiz || {};
M.mod_aiquiz.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_aiquiz-util-base",
        "moodle-mod_aiquiz-util-page",
