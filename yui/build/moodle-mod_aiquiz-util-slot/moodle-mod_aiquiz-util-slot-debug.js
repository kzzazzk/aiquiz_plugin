YUI.add('moodle-mod_aiquiz-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module moodle-mod_aiquiz-util
 * @submodule moodle-mod_aiquiz-util-slot
 */

Y.namespace('Moodle.mod_aiquiz.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Moodle.mod_aiquiz.util.slot
 * @static
 */
Y.Moodle.mod_aiquiz.util.slot = {
    CSS: {
        SLOT: 'slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Moodle.mod_aiquiz.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Moodle.mod_aiquiz.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    remove: function(slot) {
        var page = Y.Moodle.mod_aiquiz.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Moodle.mod_aiquiz.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Moodle.mod_aiquiz.util.page.remove(page);
    },
    ...
    addPageBreak: function(slot) {
        var nodetext = M.mod_aiquiz.resource_toolbox.get('config').addpageiconhtml;
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '';
            var iconname = '';
            if (Y.Moodle.mod_aiquiz.util.page.isPage(nextitem)) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'aiquiz'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'aiquiz'));
            icon.set('alt', M.util.get_string(action, 'aiquiz'));
    ...
    updateDependencyIcon: function(slot, requiresprevious) {
        ...
        if (requiresprevious) {
            link.set('title', M.util.get_string('questiondependencyremove', 'aiquiz', a));
            link.setData('action', 'removedependency');
            window.require(['core/templates'], function(Templates) {
                Templates.renderPix('t/locked', 'core', M.util.get_string('questiondependsonprevious', 'aiquiz')).then(
                    function(html) {
                        icon.replace(html);
                    }
                );
            ...
            link.set('title', M.util.get_string('questiondependencyadd', 'aiquiz', a));
            link.setData('action', 'adddependency');
            window.require(['core/templates'], function(Templates) {
                Templates.renderPix('t/unlocked', 'core', M.util.get_string('questiondependencyfree', 'aiquiz')).then(
                    function(html) {
                        icon.replace(html);
                    }
                );
...
}, '@VERSION@', {"requires": ["node", "moodle-mod_aiquiz-util-base"]});
