YUI.add('moodle-mod_aiquiz-autosave', function (Y, NAME) {
...
/**
 * Auto-save functionality for during aiquiz attempts.
 *
 * @module moodle-mod_aiquiz-autosave
 */

/**
 * Auto-save functionality for during aiquiz attempts.
 *
 * @class M.mod_aiquiz.autosave
 */

M.mod_aiquiz = M.mod_aiquiz || {};
M.mod_aiquiz.autosave = {
    /**
     ...
     * @type String
     * @default M.cfg.wwwroot + '/mod/aiquiz/autosave.ajax.php'
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/aiquiz/autosave.ajax.php',
     ...
     */
    init: function(delay) {
        this.form = Y.one(this.SELECTORS.aiquiz_FORM);
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-mod_aiquiz-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof window.tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-mod_aiquiz-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-mod_aiquiz-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-mod_aiquiz-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-mod_aiquiz-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-mod_aiquiz-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-mod_aiquiz-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-mod_aiquiz-autosave');
        if (typeof window.tinyMCE !== 'undefined') {
    ...
    save_done: function(transactionid, response) {
        ...
        if (typeof autosavedata.timeleft !== 'undefined') {
            Y.log('Updating timer: ' + autosavedata.timeleft + ' seconds remain.', 'debug', 'moodle-mod_aiquiz-timer');
            M.mod_aiquiz.timer.updateEndTime(autosavedata.timeleft);
        }

        Y.log('Save completed.', 'debug', 'moodle-mod_aiquiz-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-mod_aiquiz-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-mod_aiquiz-autosave');
        this.save_transaction = null;
    ...
    is_time_nearly_over: function() {
        return M.mod_aiquiz.timer && M.mod_aiquiz.timer.endtime &&
                (new Date().getTime() + 2 * this.delay) > M.mod_aiquiz.timer.endtime;
    },
