YUI.add('moodle-mod_aiquiz-util-page', function (Y, NAME) {
 ...
 * @submodule moodle-mod_aiquiz-util-page
 */

Y.namespace('Moodle.mod_aiquiz.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_aiquiz.util.page
 * @static
 */
Y.Moodle.mod_aiquiz.util.page = {
    CSS: {
        PAGE: 'page'
    },
    CONSTANTS: {
     ...
     */
    getPages: function() {
        return Y.all(Y.Moodle.mod_aiquiz.util.slot.SELECTORS.PAGECONTENT + ' ' +
                     Y.Moodle.mod_aiquiz.util.slot.SELECTORS.SECTIONUL + ' ' +
                    this.SELECTORS.PAGE);
    },
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_aiquiz.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_aiquiz.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_aiquiz.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_aiquiz.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
