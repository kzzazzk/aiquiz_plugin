YUI.add('moodle-mod_aiquiz-toolboxes', function (Y, NAME) {
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'aiquiz', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a aiquiz in editing mode.
 *
 * @module mod_aiquiz-resource-toolbox
 * @namespace M.mod_aiquiz.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_aiquiz.quizbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_aiquiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_aiquiz.util.slot.remove(element);
    ...
    delete_multiple_action: function(ev) {
        ...
        if (typeof problemsection !== 'undefined') {
            var alert = new M.core.alert({
                title: M.util.get_string('cannotremoveslots', 'aiquiz'),
                message: M.util.get_string('cannotremoveallsectionslots', 'aiquiz', problemsection)
            });
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_aiquiz.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_aiquiz.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_aiquiz.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_aiquiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_aiquiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_aiquiz.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_aiquiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_aiquiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_aiquiz.util.slot.reorderSlots();
        Y.Moodle.mod_aiquiz.util.slot.reorderPageBreaks();
        Y.Moodle.mod_aiquiz.util.page.reorderPages();
        Y.Moodle.mod_aiquiz.util.slot.updateOneSlotSections();
        Y.Moodle.mod_aiquiz.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_aiquiz-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_aiquiz.resource_toolbox = null;
M.mod_aiquiz.init_resource_toolbox = function(config) {
    M.mod_aiquiz.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_aiquiz.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_aiquiz-toolboxes
 * @namespace M.mod_aiquiz.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a aiquiz.
 *
 * @class section
 * @constructor
 * @extends M.mod_aiquiz.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'aiquiz', activity.getData('sectionname')),
            modal: true
        });
    ...
    edit_section_title_submit: function(ev, activity, oldtext) {
        ...
        if (newtext !== null && newtext !== oldtext) {
            ...
            if (newtext.trim() === '') {
                ...
                instancesectiontext = M.util.get_string('sectionnoname', 'aiquiz');
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'aiquiz', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'aiquiz', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'aiquiz', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'aiquiz', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_aiquiz-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_aiquiz.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_aiquiz-quizbase",
        "moodle-mod_aiquiz-util-slot",
